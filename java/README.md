...
|---|---|
| `firms().createConsentOffer(request)` | `POST /consent-offers` |
| `firms().getConsentOffer(id)` | `GET /consent-offers/{offerId}` |
| `whiteLabel().createCustomer(request, key)` | `POST /customers` |
| `whiteLabel().listCustomers(...)` | `GET /customers` |
...
- Send responses include replay/link metadata. Document events now use the live
...
| `webhooks().queue().pull(...)`        | GET    | `/events/pull`                     |
