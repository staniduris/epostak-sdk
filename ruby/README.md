...
|---|---|
| `firms.create_consent_offer(...)` | `POST /consent-offers` |
| `firms.get_consent_offer(id)` | `GET /consent-offers/{offerId}` |
| `white_label.create_customer(...)` | `POST /customers` |
| `white_label.list_customers(...)` | `GET /customers` |
...
operation = client.white_label.register_participant(
  {
    ...
    verificationToken: fs_webhook.fetch("verification_token")
  },
  idempotency_key: "fs-webhook:event-123"
)
status = client.white_label.get_operation(operation.fetch("id"))
```
