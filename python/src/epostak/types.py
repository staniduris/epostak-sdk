...
class IntegratorPricingTier(TypedDict, total=False):
    ...
    The last entry has ``upTo`` set to ``None`` and carries the public
    marginal rate. ``contactRequired`` states whether a custom order is needed.
    """

    upTo: Optional[int]  # Inclusive upper bound, ``None`` for the open tier
    rate: float  # Per-document rate in EUR
    label: str  # Human-readable label (e.g. "20 001+")
...
class IntegratorBillableUsage(TypedDict):
    ...
    apiUsageCharge: float
    whiteLabelBaseCharge: float
    whiteLabelOverageCharge: float
    totalCharge: float  # API usage plus contract charges, cents-rounded
    exceedsAutoTier: bool
    currency: str  # "EUR"


class IntegratorSandboxUsage(TypedDict):
    """Non-billable usage generated by managed sandbox firms."""

    firms: int
    outboundCount: int
    inboundApiCount: int
    outboundCharge: float
    inboundApiCharge: float
    totalCharge: float
    exceedsAutoTier: bool
    currency: str


class IntegratorRebillingEstimate(TypedDict):
    currency: str
    pricingMethod: str
    effectiveOutboundRate: Optional[float]
    effectiveInboundApiRate: Optional[float]
    outboundAmount: Optional[float]
    inboundApiAmount: Optional[float]
    totalAmount: Optional[float]
    provisional: bool
    includedInCurrentBillableAggregate: bool
...
class IntegratorFirmUsage(TypedDict):
    ...
    outboundCount: int
    inboundApiCount: int
    rebillingEstimate: IntegratorRebillingEstimate
...
class _IntegratorPricing(TypedDict):
    scheduleVersion: str
    model: str  # "progressive"
    currency: str  # "EUR"
    thresholdScope: str  # "combined" or "per_direction"
    contactThreshold: int
    marginalBandStartsAt: int
