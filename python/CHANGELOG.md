...
- Added the typed `white_label` participant lifecycle for FS SR token
...
- Added typed `firms.create_consent_link()` support for the Enterprise 1.8.6
...
- Synced Enterprise 1.6.0 JSON self-billing/process fields, idempotent send
  response metadata with live camelCase keys, document `process_id`, and nested
  event pagination.
...
- Canonical `client.payloads`, `client.events`, and `support_packet()` are now
  the default Enterprise API path.
...
- `EPostakError` now exposes `field`, `next_action`, `retryable`, `request_id`,
  and `retry_after`; business events include `document.cancelled`.
- Added live JSON billing payload fields to `SendDocumentRequest` typing:
  ...
  fields.
- Split typed send payloads into JSON/XML request shapes so JSON mode requires
  `receiverName` and does not advertise non-live `docType`.
...
- Connector invoice responses omit an optional `note` when its runtime value
  is `None`, matching the API's non-null string contract.
