...
export interface IntegratorPricingTier {
  /** Inclusive upper bound of the tier (in documents), or `null` on the
   *  open-ended marginal band. */
  upTo: number | null;
  /** Per-document rate in EUR. */
  rate: number;
  /** Human-readable tier label (for example `"20 001+"`). */
  label?: string;
  /** Whether this tier requires a custom sales contract. */
...
export interface IntegratorBillableUsage {
  managedFirms: number;
  /** Maximum billable active-firm count observed in the billing period. */
  maximumActiveFirms: number;
  ...
  /** Sum of outbound and inbound API usage charges. */
  apiUsageCharge: number;
  /** Contractual white-label base charge for the period. */
  whiteLabelBaseCharge: number;
  /** White-label active-firm overage charge for the period. */
  whiteLabelOverageCharge: number;
  /** Total API and contract charge, rounded to cents. */
  totalCharge: number;
  /** Whether the signed pricing snapshot requires a custom order above its threshold. */
  exceedsAutoTier: boolean;
  currency: "EUR";
}

/** Non-billable usage generated by managed sandbox firms. */
export interface IntegratorSandboxUsage {
  firms: number;
  outboundCount: number;
  inboundApiCount: number;
  outboundCharge: number;
...
export interface IntegratorFirmUsage {
  ...
  managed: boolean;
  /** `true` when the firm is an intentionally non-billable sandbox. */
  sandbox: boolean;
  /** `true` when the firm contributes to the current billed aggregate. */
  billable: boolean;
  outboundCount: number;
  inboundApiCount: number;
  rebillingEstimate: {
    currency: "EUR";
    pricingMethod: "blended_aggregate_rate";
    effectiveOutboundRate: number | null;
    effectiveInboundApiRate: number | null;
    outboundAmount: number | null;
    inboundApiAmount: number | null;
    totalAmount: number | null;
    provisional: true;
    includedInCurrentBillableAggregate: boolean;
...
export interface IntegratorLicenseInfo {
  ...
  /** Managed sandbox usage, priced for visibility but excluded from billing. */
  sandbox: IntegratorSandboxUsage;
  /** Estimate if managed sandbox firms were included in production billing. */
  productionEstimate: IntegratorBillableUsage;
  nonManaged: IntegratorNonManagedUsage;
  /** Historical signed-schedule flag. It remains `false` for V1.2 because
   *  the 20,001+ marginal band has a public rate. */
  exceedsAutoTier: boolean;
  /** Compatibility threshold from the signed schedule; inspect the tier's
   *  `contactRequired` flag to determine whether a custom order is needed. */
  contactThreshold: number;
  pricing: {
    scheduleVersion: string;
    model: "progressive";
    currency: "EUR";
    thresholdScope: "combined" | "per_direction";
    contactThreshold: number;
    marginalBandStartsAt: number;
